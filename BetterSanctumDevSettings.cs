...
public class RunTrackingSettings
{
    [JsonIgnore]
    public CustomNode Help { get; set; } = SettingsHelp.Block(
        "Records one run at a time and writes it on End Run to Logs/BetterSanctumDev/tracking/<tracking list>/: sanctum-runs.csv holds a row per run, sanctum-run-rooms.csv a row per room and reward slot, sanctum-run-currency.csv the run row again as one row per currency, and sanctum-run-wide.csv a row per run with a column per currency.",
        "sanctum-deals.csv sits above those, in Logs/BetterSanctumDev/, and every run adds to it whichever list is selected. Its columns are fixed, so nothing in it depends on which currencies a list tracks, and what a deal is worth is a question answered by volume rather than by keeping the runs apart.",
